...
# 테이블 페이지에서 수정 버튼 클릭 시 해당 월로 자동 이동
if "goto_year" in st.session_state:
    st.session_state["sel_year"] = st.session_state.pop("goto_year")
if "goto_month" in st.session_state:
    st.session_state["sel_month"] = st.session_state.pop("goto_month")

with st.sidebar:
    st.markdown("### 📅 월 선택")
    sel_year = st.selectbox(
        "년도", year_options,
        index=year_options.index(st.session_state.get("sel_year", today.year)),
        key="sel_year",
    )
    sel_month = st.selectbox(
        "월", month_options,
        index=month_options.index(st.session_state.get("sel_month", f"{today.month:02d}")),
        key="sel_month",
    )
...
is_edit_mode = year_month in all_data
existing = all_data.get(year_month, get_empty_month())

# 수정 모드 배너
if is_edit_mode:
    st.warning(f"✏️ **{year_month}** 데이터가 이미 존재합니다. 값을 수정한 뒤 저장하세요.", icon="⚠️")
else:
...
# ── 저장 ─────────────────────────────────────────────────────────────────────
btn_label = "💾 수정 저장" if is_edit_mode else "💾 저장"
if st.button(btn_label, type="primary", use_container_width=True):
    ...
    if save_month_data(year_month, month_data):
        st.success(f"✅ {year_month} 데이터가 {'수정' if is_edit_mode else '저장'}되었습니다!")
        st.balloons()
    else:
